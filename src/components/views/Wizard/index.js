...
const Form = ({ form, compile, run }) => (
  <form id={form}>
    ...
    >
      ...
      />
      <Editor
        title='PC_bsf_ParametersOutput(int numOfWorkers, PT_bsf_data_T data): выводит начальные параметры задачи, используя аргументы numOfWorkers – количество процессов-рабочих и data – начальное задание, содержащее начальное приближение;* PC_bsf_ProblemOutput(PT_bsf_reduceElem_T* reduceResult, int count, PT_bsf_data_T data, int iterCount, double t): выводит конечные результаты работы программы, используя аргументы *reduceResult – результат выполнения оператора Reduce, count количество элементов «суммированных» при выполнении оператора Reduce, data – последнее задание (последнее приближение), t – общее время работы программы'
